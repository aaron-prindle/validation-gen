...
const (
	subListMapItemTagName = "k8s:subListMapItem"
)

func init() {
	RegisterTagValidator(&subListMapItemTagValidator{})
}

type subListMapItemTagValidator struct {
	validator Validator
}

func (stv *subListMapItemTagValidator) Init(cfg Config) {
	stv.validator = cfg.Validator
}

func (subListMapItemTagValidator) TagName() string {
	return subListMapItemTagName
}

var subListMapItemTagValidScopes = sets.New(ScopeAny)

func (subListMapItemTagValidator) ValidScopes() sets.Set[Scope] {
	return subListMapItemTagValidScopes
}

type parsedSubListMapItemKeyValues struct {
	MatcherMap map[string]string
}

var (
	validateSubListMapItemByKeyValues = types.Name{Package: libValidationPkg, Name: "SubListMapItemByKeyValues"}
)

func (stv *subListMapItemTagValidator) GetValidations(context Context, args []string, payload string) (Validations, error) {
	if len(args) != 1 {
		return Validations{}, fmt.Errorf("requires exactly one arg")
	}
	configStr := args[0]

	parsedArg, err := parseSubListMapItemArg(configStr)
	if err != nil {
	...
	if len(listMapKey) == 0 {
		return Validations{}, fmt.Errorf("must have '+listType=map' and '+listMapKey=...' annotations to use subListMapItem with matcher map")
	}
	if _, ok := parsedArg.MatcherMap[listMapKey]; !ok {
		return Validations{}, fmt.Errorf("subListMapItem matcher map must contain listMap key")
	}
	...
	if validations, err := stv.validator.ExtractValidations(subContext, fakeComments); err != nil {
		...
		for _, vfn := range validations.Functions {
			f := Function(
				subListMapItemTagName,
				vfn.Flags,
				validateSubListMapItemByKeyValues,
				matchFn,
...
func parseSubListMapItemArg(argStr string) (*parsedSubListMapItemKeyValues, error) {
	var matcherMap map[string]string
	if err := json.Unmarshal([]byte(argStr), &matcherMap); err == nil {
		if len(matcherMap) == 0 {
			return nil, fmt.Errorf("subListMapItem JSON matcher map cannot be empty")
		}

		return &parsedSubListMapItemKeyValues{
			MatcherMap: matcherMap,
		}, nil
	}
	return nil, fmt.Errorf("subListMapItem arguments incorrect, JSON parsing failed")
}
...
func (stv subListMapItemTagValidator) Docs() TagDoc {
	doc := TagDoc{
		Tag:    stv.TagName(),
		Scopes: stv.ValidScopes().UnsortedList(),
		Description: "Declares a validation for a subListMapItem targeting a single struct item in a slice/list with listType=map" +
			"matching specified criteria. One of the match criteria must be on value of the listMapKey.",
		Args: []TagArgDoc{
			{
				Description: `{"<list-map-key-field-json-name>":"<value>", "<optional-additional-field-json-name>": "<value>", ...}`,
			},
		},
		Docs: "The JSON map argument must contain the listMapKey json field name as one of the keys.",
