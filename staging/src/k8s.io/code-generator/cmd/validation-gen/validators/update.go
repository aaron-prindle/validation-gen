...
const (
	...
	// Update constraints for non-list fields
...
type updateFieldMetadata struct {
	updateConstraints []string // NoSet, NoUnset, NoModify
}
...
func (utc updateTagCollector) validateConstraintsForType(context Context, constraints []string) error {
	...
	for _, constraint := range constraints {
		switch constraint {
		case constraintNoSet, constraintNoUnset, constraintNoModify:
			// All these constraints CANNOT be used on lists/maps
			if isCompound {
...
func (ufv updateFieldValidator) GetValidations(context Context) (Validations, error) {
	...
	// Check if this is a compound type - if so, no validators should have been allowed
	t := util.NonPointer(util.NativeType(context.Type))
	if t.Kind == types.Slice || t.Kind == types.Map {
		// This should have been caught during tag validation, but double-check
		return Validations{}, fmt.Errorf("update constraints are not supported on list or map fields")
	}
...
func (ufv updateFieldValidator) generateValidations(
	...
	// Add validators based on capabilities
