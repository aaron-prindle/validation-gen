...
func isSyntheticListMember(memberAny any) (*types.Member, bool) {
	var m *types.Member
	...
	return m, strings.HasPrefix(m.Name, syntheticMemberPrefix)
}

func parseSyntheticMemberMetadata(syntheticMemberName string, elemType *types.Type) (string, map[string]string, error) {
	if !strings.HasPrefix(syntheticMemberName, syntheticMemberPrefix) {
		return "", nil, fmt.Errorf("not a synthetic member name: %s", syntheticMemberName)
	}
	trimmedName := strings.TrimPrefix(syntheticMemberName, syntheticMemberPrefix)
	parts := strings.SplitN(trimmedName, "_", 2)
	if len(parts) < 2 {
		return "", nil, fmt.Errorf("invalid synthetic member name format (missing list field or selector): %s", syntheticMemberName)
	}
	...
	for _, sp := range selectorParts {
		...
		if len(kv) != 2 {
			return "", nil, fmt.Errorf("invalid selector part in synthetic member name '%s': part '%s'", syntheticMemberName, sp)
		}
		jsonKey := kv[0]
		value := kv[1]
		if getMemberByJSON(elemType, jsonKey) == nil {
			return "", nil, fmt.Errorf("synthetic member selector key '%s' not found as JSON field in element type '%s'", jsonKey, elemType.Name)
		}
		selectorMap[jsonKey] = value
	}
	if len(selectorMap) == 0 {
		return "", nil, fmt.Errorf("no selector criteria parsed from synthetic member name: %s", syntheticMemberName)
	}
...
func (utv unionTypeValidator) GetValidations(context Context) (Validations, error) {
	...
	for _, unionName := range unionNames {
		...
		for _, memberAny := range u.fieldMembers {
			...
			if syntheticMember, isSynthetic := isSyntheticListMember(currentMember); isSynthetic {
				elemType := syntheticMember.Type
				if elemType == nil {
					return Validations{}, fmt.Errorf("synthetic member '%s' for union '%s' has nil Type", syntheticMember.Name, unionName)
				}

				listFieldName, selectorMap, err := parseSyntheticMemberMetadata(syntheticMember.Name, elemType)
				if err != nil {
					return Validations{}, fmt.Errorf("error parsing metadata for synthetic member '%s' (union '%s'): %w", syntheticMember.Name, unionName, err)
				}

				listFieldInParent := getMemberPtr(structType, listFieldName)
				if listFieldInParent == nil {
					return Validations{}, fmt.Errorf("list field '%s' (from synthetic member '%s') not found in struct '%s'", listFieldName, syntheticMember.Name, structType.Name)
				}
				...
				for jsonKeyFromSelector, expectedValue := range selectorMap {
					...
					if err_gf != nil {
						return Validations{}, fmt.Errorf("error generating union validation for '%s' (synthetic member '%s', selector key '%s'): %w", unionName, syntheticMember.Name, jsonKeyFromSelector, err_gf)
					}
...
func (umtv unionMemberTagValidator) GetValidations(context Context, _ []string, payload string) (Validations, error) {
	if context.Member == nil {
		return Validations{}, fmt.Errorf("%s: context.Member is nil. This should not happen if called via subfield that constructs a synthetic member, or directly on a field.", umtv.TagName())
	}
