...
func init() {
	...
	// shared maps from field path strings to union definitions
	// This allows the same type to have different unions in different contexts
...
func (utfv unionTypeOrFieldValidator) GetValidations(context Context) (Validations, error) {
	...
	for _, unionName := range keys {
		...
		if len(u.fieldMembers) > 0 || u.discriminator != nil || len(u.itemMatchers) > 0 {
			...
			varBaseName := sanitizeName(strings.ReplaceAll(context.Path.String(), ".", "_") + "_" + unionName)
			supportVarName := PrivateVar{Name: "unionMembershipFor_" + varBaseName, Package: "local"}
			...
			if context.Type.Kind == types.Slice && len(u.itemMatchers) > 0 {
				...
				for _, fullPath := range matcherPaths {
					...
					// Generate a cleaner extractor that checks for item existence
					extractor.Body = fmt.Sprintf(`var matched *%s
validate.SliceItem(ctx, op, fldPath, list, nil, func(item *%s) bool { return %s }, func(ctx context.Context, op operation.Operation, itemPath *field.Path, newItem, oldItem *%s) field.ErrorList { matched = newItem; return nil })
return matched != nil`, elemType.Name.Name, elemType.Name.Name, strings.Join(conditions, " && "), elemType.Name.Name)
...
// For item-based unions, it converts paths like "Pipeline.Tasks[{\"name\": \"succeeded\"}]"
...
func (udtv unionDiscriminatorTagValidator) GetValidations(context Context, tag codetags.Tag) (Validations, error) {
	...
	if udtv.shared[context.ParentPath.String()] == nil {
		udtv.shared[context.ParentPath.String()] = unions{}
	}
	unionArg, _ := tag.NamedArg("union") // optional
	u := udtv.shared[context.ParentPath.String()].getOrCreate(unionArg.Value)
...
func (umtv unionMemberTagValidator) GetValidations(context Context, tag codetags.Tag) (Validations, error) {
	var fieldName string // TODO: Avoid the "local" here. This was added to to avoid errors caused when the package is an empty string.
	//       The correct package would be the output package but is not known here. This does not show up in generated code.
	// TODO: Append a consistent hash suffix to avoid generated name conflicts?
	var pp string
