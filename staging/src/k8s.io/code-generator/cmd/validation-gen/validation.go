...
func toGolangSourceDataLiteral(sw *generator.SnippetWriter, c *generator.Context, value any) {
	...
	case validators.WrapperFunction:
		if extraArgs := v.Function.Args; len(extraArgs) == 0 {
			// Check if we need signature adaptation based on validation level
			if v.Function.Level == validators.ValidationLevelCollection && isSliceType(v.ObjType) {
				// Collection-level validators expect slice arguments, not pointers to slices
				targs := generator.Args{
				...
				sw.Do("op $.operation.Operation|raw$, ", targs)
				sw.Do("fldPath *$.field.Path|raw$, ", targs)
				sw.Do("obj, oldObj *$.objType|raw$) $.field.ErrorList|raw$ {\n", targs)
				sw.Do("    var newSlice, oldSlice $.objType|raw$\n", targs)
				sw.Do("    if obj != nil { newSlice = *obj }\n", targs)
				sw.Do("    if oldObj != nil { oldSlice = *oldObj }\n", targs)
				sw.Do("    return $.funcName|raw$(ctx, op, fldPath, newSlice, oldSlice)\n", targs)
				sw.Do("}", targs)
				return
			}

			// Default: use the function directly
			targs := generator.Args{
				"funcName": c.Universe.Type(v.Function.Function),
			}
			sw.Do("$.funcName|raw$", targs)
...
// isSliceType returns true if the type is a slice or array
func isSliceType(t *types.Type) bool {
	return t.Kind == types.Slice || t.Kind == types.Array
}
