...
var (
	...
	operationPkgSymbols = mkPkgNames(operationPkg, "Operation", "Update")
	contextPkg          = "context"
	...
	validatePkg         = "k8s.io/apimachinery/pkg/api/validate"
	// Add these new variables
	validatePkgSymbols   = mkPkgNames(validatePkg, "EachMapSliceVal")
	validateEachMapVal   = types.Name{Package: validatePkg, Name: "EachMapVal"}
	validateEachSliceVal = types.Name{Package: validatePkg, Name: "EachSliceVal"}
)
		...
		for _, fld := range thisNode.fields {
			...
			if fld.node != nil {
				...
				case types.Map:
					...
					// If this field is a map and the value-type has validations,
					// call its validation function.
					if validations := fld.fieldValIterations; !validations.Empty() {
						emitComments(validations.Comments, bufsw)
						if !fldRatchetingChecked {
							emitRatchetingCheck(c, fld.childType, bufsw)
							fldRatchetingChecked = true
					...
					// Don't descend into map fields here - validations are handled above
					// OLD vvv
					// Descend into this field.
					// g.emitValidationForChild(c, fld, bufsw)
				default:
...
func emitMapSliceValidator(c *generator.Context, v validators.FunctionGen, spec validators.MapSliceValidationSpec, sw *generator.SnippetWriter) {
	...
	// Emit slice-level validations (e.g., maxItems)
	...
	if len(spec.ElementValidations) > 0 {
		...
		// Check if we have a single validation function that matches the element type
		hasNamedValidator := false
		for _, ev := range spec.ElementValidations {
			// Check if this is a validation function for the element type
			if ev.Function.Name != "" && strings.HasPrefix(ev.Function.Name, "Validate_") {
				hasNamedValidator = true
				break
			}
		}
		...
		if hasNamedValidator && len(spec.ElementValidations) == 1 {
			ev := spec.ElementValidations[0]
			// Direct call to the validation function
			sw.Do("$.|raw$", c.Universe.Type(ev.Function))
		} else {
			// Generate inline validation function
			...
			// If there are any validations, emit them
			if len(spec.ElementValidations) > 0 {
				// Check if we need ratcheting for builtin types
				if validators.IsDirectComparable(validators.NonPointer(validators.NativeType(spec.ElementType))) {
				...
				if len(spec.ElementValidations) == 1 {
					...
					for _, ev := range spec.ElementValidations {
						evTargs := generator.Args{
							"funcName": c.Universe.Type(ev.Function),
						}
						sw.Do("            errs = append(errs, $.funcName|raw$(ctx, op, fldPath, obj, oldObj", evTargs)
						for _, arg := range ev.Args {
							sw.Do(", ", nil)
							toGolangSourceDataLiteral(sw, c, arg)
						}
